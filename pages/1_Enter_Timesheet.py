...
def progress_count():
    ...
    for r in st.session_state.get("entries", []):
        if any([r["rs_on"], r["as_on"], r["rs_off"], r["as_off"], r["worked"], r["extra"], r["sick"], r["off"], r["ado"]]):
            cnt += 1
    return cnt
...
with st.form("day_form", clear_on_submit=False):
    ...
    c5, c6 = st.columns(2)
    worked = c5.text_input("Worked", value=row["worked"], key=f'worked_{day_index}', help="HH:MM or HHMM (blank → 8h)")
    extra  = c6.text_input("Extra",  value=row["extra"],  key=f'extra_{day_index}', help="HH:MM or HHMM")

    t1, t2, t3 = st.columns(3)
    sick_chk = t1.checkbox("Sick", value=row["sick"], key=f'sick_{day_index}', help="Behaves like OFF for unit; also adds Sick rate")
    off_chk  = t2.checkbox("Off",  value=row["off"],  key=f'off_{day_index}', help='Acts like entering "OFF"')
    ado_chk  = t3.checkbox("ADO",  value=row["ado"],  key=f'ado_{day_index}', help='Acts like entering "ADO"')

    # convenience: copy previous day's inputs
    copy_col, _, jump_col = st.columns([1,2,1])
...
# If copy previous toggled, prefill fields immediately and show updated inputs
...
with nav_left:
    st.button("⬅️ Previous Day", use_container_width=True, disabled=(day_index == 0),
              on_click=lambda: st.session_state.update(day_index=day_index-1))
with nav_right:
    st.button("Next Day ➡️", use_container_width=True, disabled=(day_index == total_days-1),
              on_click=lambda: st.session_state.update(day_index=day_index+1))
